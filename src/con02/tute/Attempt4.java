...
class Attempt4 implements Runnable {
    ...
    // Constructor
    public Attempt4(String threadName) {
    ...
    public void run() {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            while (turn.get(this.oppThread) != 0) {
                turn.put(this.threadName, 0); // p/q = 0
                turn.put(this.threadName, 1); // p/q = 1
            }
            // end of pre-protocol
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < Integer.parseInt(args[0]); ++i) {
            ...
            Thread p = new Thread(new Attempt4("p"));
            Thread q = new Thread(new Attempt4("q"));
            p.start();
            q.start();
            try {
                p.join();
